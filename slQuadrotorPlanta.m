function slQUadrotorPlanta(block)
...
%         X(12,1) = wz;    Z-angular velocity of B frame w.r.t. I, expressed in B
  ...
  block.OutputPort(1).Data = X;             %State
  block.OutputPort(2).Data = X(1:3);        %Position (I)
  block.OutputPort(3).Data = X(4:6);        %Velocity (B)
  block.OutputPort(4).Data = X(7:9);        %Actitude (Euler Angles) (I)
  block.OutputPort(5).Data = X(10:12);      %Angular Velocity (B)
...
X=block.ContStates.Data;
Control =  block.InputPort(1).Data;
%Thrust = block.InputPort(2).Data;


%% CALL function RigidBodyDynamics
global Quadrotor;
global Atmosphere;
global Aero_Model;
global CtrlParam;
deltaR=CtrlParam.deltaR;
[Aero_Forces, Aero_Moments_CM] = Quadrotor_Aerodynamics(Atmosphere, Quadrotor, X, Control, Aero_Model,deltaR);
[Gravity_Force] = Gravity_Forces(Atmosphere, Quadrotor, X);

    Net_Forces      = Gravity_Force + Aero_Forces;
    Net_Moments_CM  = zeros(1,3) + Aero_Moments_CM;
  

Xp = RigidBodyDynamics(X, Quadrotor.Mass_prop, Net_Forces, Net_Moments_CM);

block.Derivatives.Data = Xp;
